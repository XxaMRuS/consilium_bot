...
async def submit_complex_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if DEBUG:
        print("DEBUG: submit_complex_callback вызвана")
...
async def submit_result_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if DEBUG:
        print("DEBUG: submit_result_input вызвана")
...
async def submit_video_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if DEBUG:
        print("DEBUG: submit_video_input вызвана")
...
async def submit_comment_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if DEBUG:
        print("DEBUG: submit_comment_input вызвана")
...
async def submit_comment_skip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if DEBUG:
        print("DEBUG: submit_comment_skip вызвана")
    if context.user_data.get('conversation_state') != AWAIT_SUBMIT_COMMENT:
        return
...
async def finalize_submit(update: Update, context: ContextTypes.DEFAULT_TYPE, comment):
    if DEBUG:
        print("DEBUG: finalize_submit вызвана")
    ...
    bot = context.bot
    sent = await bot.send_message(chat_id=channel_id, text=publish_text, parse_mode='Markdown', reply_to_message_id=channel_post_id)
    comment_keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("💬 Комментировать", callback_data=f"comment_{sent.message_id}")]])
    await bot.edit_message_reply_markup(chat_id=channel_id, message_id=sent.message_id, reply_markup=comment_keyboard)
